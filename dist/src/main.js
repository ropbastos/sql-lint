...
exports.default = async ({ sql, host, port, user = "", prefix = "", password = "", database = "", verbosity = 0, driver = "mysql", }) => {
    const printer = new printer_1.Printer(verbosity, new json_1.JsonFormat());
    let db;
    if (host) {
        db = databaseFactory_1.default(driver, host, user, password, database, port);
    }
